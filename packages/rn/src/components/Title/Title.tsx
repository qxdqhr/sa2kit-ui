...
function Ribbon({ children, fontSize, color = 'default' }: { children: ReactNode; fontSize: number; color?: TitleColor }) {
  return (
    <View className={cn('ai-title-ribbon', color !== 'default' && colorClass[color])}>
      ...
      <Text className="ai-title-ribbon-text" style={{ fontSize }}>{children}</Text>
    </View>
  );
}
...
export function Title({ children, size = 'middle', color = 'default', className, style }: TitleProps) {
  return (
    <View className={cn('ai-title', className)} style={style as object}>
